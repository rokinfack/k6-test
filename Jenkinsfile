pipeline{
    ...
    stages{
        stage('Version de k6'){
         ...
         stage('Run tests'){
            steps{
                sh "k6 run --out influxdb=http://InfluxDB:InfluxDB@InfluxDB:8086/InfluxDB --stage ${params.MONTEE_STAGE_1}:${params.MAINTIEN_STAGE_2}  --stage ${params.MAINTIEN_STAGE_3}:${params.MAINTIEN_STAGE_4} --vus ${params.VU_COUNT}  ${params.CHOICE}"
            }
        }
    }
    post {
